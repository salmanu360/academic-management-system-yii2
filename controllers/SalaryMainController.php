...
class SalaryMainController extends Controller
{
    ...
    public function actionGetEmployee(){
            ...
            ->where(['user.fk_branch_id'=>yii::$app->common->getBranch(),'user.status'=>'active','employee_info.department_type_id'=>$id,'employee_info.is_active'=>1])->asArray()->all();
            $stuArray = ArrayHelper::map($warden,'emp_id','name');
            if(count($stuArray)>0){
            $viewdepartment=$this->renderAjax('getemloyee-salary',['salry'=>$warden,'dep_id'=>$id]);
            }else{
             $viewdepartment = '<div class="col-md-6" style="color:red">No Record Found against this department</div>';
            }
   ...
   /*====== show salary of employee====*/
    public function actionShowSalaryPay(){
     ...
     $currentDate=date('d');
     $totalDayGet=$totaldaysMonth - $currentDate;
     $model = new SalaryMain();
    ...
    if(count($exist)>0){
     ...
     $employee_payroll = EmployeePayroll::find()->where(['fk_emp_id'=>$empid])->one();
            ...
            $payrollView=$this->renderAjax('emp-payrollshow',['empid'=>$empid,'emply_alwnc'=>$emply_alwnc,'employee_payroll'=>$employee_payroll,'payrollDeduction'=>$payrollDeduction,'model'=>$model,'totalDayGet'=>$totalDayGet]);
            }else{
             $payrollView = '<div class="col-md-6" style="color:red">No Record Found against this department</div>';
            }
  ...
  foreach ($employee as $getEmp) {
     ...
     $employeeLeaveCount=EmployeeAttendance::find()->where(['fk_empl_id'=>$getEmp['emp_id'],'leave_type'=>'leave'])->all();
      $employeelatecomingCount=EmployeeAttendance::find()->where(['fk_empl_id'=>$getEmp['emp_id'],'leave_type'=>'late'])->all();
