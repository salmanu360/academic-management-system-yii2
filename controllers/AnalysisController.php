...
class AnalysisController extends Controller
{
    ...
    public function actionGetSectionPdf(){
       ...
       $searchModel->is_active=1;
                ...
                $StudentQuery = StudentInfo::find()
                 ...
                 $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
                 $dataProvider->pagination  = false;
                 $viewx= $this->renderAjax('section-pdf', [
                    'searchModel'       => $searchModel,
                    'dataProvider'      => $dataProvider,
                    'class_val'          => $class_val,
    ...
    public function actionGetEmptyAwardPdf(){
       $class_val= Yii::$app->request->get('classval');
       $groupval= Yii::$app->request->get('groupval');
       $sectionval= Yii::$app->request->get('sectionval');
        $studentDetails = StudentInfo::find()
            ->select(['user_id','stu_id','roll_no','class_id','group_id','section_id'])
            ->where([
                'fk_branch_id'  =>Yii::$app->common->getBranch(),
                'class_id'   => $class_val,
                'group_id'   => ($groupval)?$groupval:null,
        ...
        $view= $this->renderAjax('emptyawardlist', [
       ...
       $this->layout = 'pdf';
        $mpdf = new mPDF();
        $mpdf->WriteHTML($view);
        $mpdf->Output('empty-awardlist-pdf-'.date("d-m-Y").'.pdf', 'D');
    }
    public function actionGetBoardPdf(){
       $class_val= Yii::$app->request->get('classval');
       $groupval= Yii::$app->request->get('groupval');
       $sectionval= Yii::$app->request->get('sectionval');
        $studentDetails = StudentInfo::find()
            ->select(['user_id','stu_id','gender_type','dob','registration_date','roll_no','class_id','group_id','section_id'])
            ->where([
                'fk_branch_id'  =>Yii::$app->common->getBranch(),
                'class_id'   => $class_val,
                'group_id'   => ($groupval)?$groupval:null,
        ...
        $view= $this->renderAjax('boardlist', [
            'studentDetails'          => $studentDetails,
            'class_val'          => $class_val,
            'sectionval'        => $sectionval,
            'groupval'          => $groupval,
       ...
       $this->layout = 'pdf';
        $mpdf = new mPDF();
        $mpdf->WriteHTML($view);
        $mpdf->Output('board-list-pdf-'.date("d-m-Y").'.pdf', 'D');
    }
...
*/
    ...
    public function actionSendWhole(){
       ...
       $stuQuery = StudentInfo::find()
            ->select(["student_info.stu_id stu_id","student_info.user_id user_id","student_info.contact_no student_contact_no","student_parents_info.contact_no contact"])
        ...
        foreach ($stuQuery as $query){
            $student_contact_no=$query['student_contact_no'];
             $contct= $query['contact'];
              $stu_id= $query['stu_id'];
               if($smsActive->status == 1){
                 if($radioValue == 'parent'){
              $parent=Yii::$app->common->SendSms($contct,$textArea,$stu_id);
              if($parent){
                $alert=1;
              }else{
                $alert=2;
             ...
             $students= Yii::$app->common->SendSms($student_contact_no,$textArea,$stu_id);
              if($students){
                 $alert=1;
              }else{
                $alert=2;
        ...
        if($alert==1){
        $classSms= "<div class='row'><div class='col-md-1 col-sm-1'></div><div class='Alert alert-danger col-sm-6'><strong>Oops..! Some thing went wrong</strong></div> </div>";
        }else{
        $classSms= "<div class='row'><div class='col-md-1 col-sm-1'></div><div class='Alert alert-success col-sm-6'><strong>Message Successfully send..!</strong></div> </div>";
        }
     return json_encode(['classSms'=>$classSms]);
    ...
    public function actionSendWholeSchool(){
       ...
       $textArea=Yii::$app->request->post('textarea');  
       echo $textArea;die;
       $stringMessage = str_replace(' ', '%20', $textArea);          
       /*$stuQuery = StudentInfo::find()
        ...
        foreach ($stuQuery as $query){
               $contct= $query['contact'];
              $stu_id= $query['stu_id'];
              Yii::$app->common->SendSms($contct,$stringMessage,$stu_id); 
    ...
    public function actionAlumni(){
        ...
        if(Yii::$app->request->post()){
            ...
            if($is_active==0){
                $student_details=StudentInfo::find()->where([
                    'class_id'=>$class_id,
    ...
    public function actionAlumnipdf(){
        ...
        if($is_active==0){
            ...
            $view= $this->renderAjax('alumni_pdf', [
            'student_details_alumni' => $student_details_alumni,
            'class_id'=>$class_id,
            'group_id'=>($group_id)?$group_id:null,
            'section_id'=>$section_id,
            'session_id'=>$session_id,
