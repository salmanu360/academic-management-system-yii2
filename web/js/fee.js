...
$(document).on('focusout','input[id^="transaction-head-amount_"]',function(){
    ...
    var transportFare = 0;//$('#input_total_transport_fare').val();
    var hostelFare = 0;//$('#input_total_hostel_fare').val();
    var absendFine = $('#input_absent_fine_fare').val();
    var total=0;
    var totalArrears=0;
    ...
    if(parseInt(payVal) >= parseInt(totalVal)){
        ...
        console.log(masterAmount+' '+extraHeadAmt);
        ...
        if(absendFine !=''){
            masterAmount = (masterAmount += parseInt(absendFine));
        }
...
$(document).on('click','#save-extra-fee-head',function () {
    ...
    if(error == 0){
        ...
        $('input[id^="transaction-head-arrears_"]').each(function(){
            ...
            html += "<th><span class='res_total'>"+$('#ex_head :selected').text().toUpperCase()+"</span></th>"; 
        ...
        if(absendFine !=''){
            totalAmtWithExtraHead = (totalAmtWithExtraHead += parseInt(absendFine));
        }
...
/*save extra head in bulk starts*/
$(document).on('click','#save-extra-fee-head-bulk',function () {
    var head = $('#ex_head');
    var headAmount = $('#ex_head_amount');
    var error = 0;
    var extraHeadAmt = 0;
    ...
    if(error == 0){
         $('#extra_head_amount_container').html('<input type="hidden" id="bulk_extra_head_ids" name="head-id[]" value="'+parseInt(head.val())+'" /><input type="hidden" name="head-amount[]" id="bulk_extra_head_amounts" value="'+parseInt(headAmount.val())+'" />');
...
function transportAdjust(event){
    if(event.keyCode != 8){
        ...
        if(error == 0){
            ...
            if(absendFine > 0){
                totalHeadAmounts =(totalHeadAmounts+parseInt(absendFine));
            }
            //console.log(totalAmount);
            /*diduct discount % amount from total.*/
...
function hostelAdjust(event){
    if(event.keyCode != 8){
        var totalAmount          = $('#net-amount').data('net');
        var hostelFare        = $('#input_total_hostel_fare').val();
        var hostelFareFareLimit   = $('#input_total_hostel_fare').data('totalhostl');
        var discountType    = $('#discount-type').val();
        var transportFare      = $('#input_total_transport_fare').val();
        var absendFine = $('#input_absent_fine_fare').val();
        var totalArrears    = 0;
        ...
        if(hostelFare > hostelFareFareLimit){
            error++;
        }
        if(error == 0){
...
$(document).on('click','#fee-submission-submit-fee-button',function(){
    $(this).attr('disabled','disabled');
    $('#fee_submission_form').submit();
});

/*============== fee submission one time get student discount fee or class by salman*/
$(document).on('change','.feeSubmissionOnetime',function(){
    //var class_id=$(this).data('id');
    var stu_id=$(this).val();
    var fee_head_id=$('#onetimeHeadforOntimesubmission').val();
    ...
    $.ajax({
            type: "POST",
            dataType:"JSON",
            data: {stu_id:stu_id,fee_head_id:fee_head_id,class_id:class_id,group:group},
            url: url,
            cache: false,
            success: function(result){
                $('#showHeadamount').val(result.headFee);
                $('#total_amount').val(result.headFee);
            }
    });
});

$(document).on('click','#submit-extra-head',function () {
  var url=$(this).data('url');
  var stu_id=$('#stu_id_extra_head').val();
  var fee_head_id=$('#ex_head').val();
  var amount=$('#ex_head_amount').val();
  $.ajax({
            type: "POST",
            dataType:"JSON",
            data: {stu_id:stu_id,fee_head_id:fee_head_id,amount:amount},
            url: url,
            cache: false,
            success: function(result){
                if(result.extraheadFee==1){    
                $('#extra_head_mesg').html('Successfully Sumbited..!');
                }else{
                $('#extra_head_mesg').html(result.msg);
                }
            }
    });
...
/*get one time fee against class for fee submission one time head*/
/* class fee group by salman*/
$(document).on('change','.onetimeheadclasswise',function(){
    var classid=$('#class-id').val();
    var groupid=$('#group-id').val();
     if(groupid ==''){ 
        group=null;
    }
    var url=$('#urloforgettingonetimehead').val();
    $.ajax({
            type: "POST",
            dataType:"JSON",
            ...
            cache: false,
            success: function(result){
                $('#onetimeHeadforOntimesubmission').html(result.options);
            }
    });
});
/*get one time fee against class for fee submission one time head ends*/
...
$(document).on('click','#previousFeeDetails',function(){
    var stu_id=$(this).data('stuid');
    var url=$(this).data('url');
    $('#loading').show();
    $.ajax({
            type: "POST",
            ...
            success: function(result){
                $('#loading').hide();
                $('#showPreviousDetails').html(result.data);
            }
    });
});