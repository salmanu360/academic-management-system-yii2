...
?>
                 <input type="hidden" id="pasclasid" value="<?=$class_id ?>">
				<input type="hidden" id="pasgroup_id" value="<?=$group_id ?>">
				<input type="hidden" id="passection_id" value="<?=$section_id ?>">
<div id="calendar_div">
	<?php echo getCalender('','',$class_id,$group_id,$section_id); 
	?>
</div>
<?php 
...
function getCalender($year = '',$month = '',$class,$group,$section)
{
	...
	<div id="calender_section" class="fullCalendar table-details attendance">
		<div class="row">
			<div class="col-md-12">
				<div id="calender_section_head">
					<h2 class="date_ctrd">
						...
						<select name="month_dropdown" class="month_dropdown dropdown" disabled="disabled">
							<?php echo getAllMonths($dateMonth); ?>
						</select>
						<select name="year_dropdown" class="year_dropdown dropdown" disabled="disabled">
							<?php echo getYearList($dateYear); ?>
						</select>
						<a href="javascript:void(0);" onclick="getCalendar('calendar_div','<?php echo date("Y",strtotime($date.' + 1 Month')); ?>','<?php echo date("m",strtotime($date.' + 1 Month')); ?>','<?php echo $class; ?>','<?php echo $group; ?>','<?php echo $section; ?>');">&gt;</a> </h2>
					</div>
				</div>
			</div>
					...
					<div class="table-responsive">
                        ...
                        //echo $dayCount;
										...
										$emp=StudentInfo::find()->where([
											...
											foreach ($emp as $q) { ?>
											<tr class="atten_d">
												<td class="atd_name"><?php echo Yii::$app->common->getName($q->user_id);?></td>
												<?php 
												$dayCount = 1; 
												for($cb=1;$cb<=$boxDisplay;$cb++){
													if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
            ...
            foreach ($attendance as $attend) {
            	...
            	if($getCurrentD == $date){
            		...
            		}else if($attend->leave_type == 'late'){
            		...
            		}else if($attend->leave_type == 'Latewithexcuse'){
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body"> 
				...
				$a= ['absent' => 'Absent', 'leave' => 'Leave','shortleave'=>'Short Leave', 'late' => 'Late','Latewithexcuse'=>'Late With Excuse','present'=>'Present'];
			...
			<div class="modal-footer">
				<button type="button" dates="<?php echo $currentDate;?>" data-url="<?= Url::to(['student/save-leave']) ?>" class="student_pop btn green-btn">Save</button>
