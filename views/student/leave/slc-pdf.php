...
use yii\helpers\Url;
$getstuid=studentInfo::find()->where(['stu_id'=>$levInfo->stu_id])->one();
$id=$levInfo->stu_id;
...
<ol>
  <li><?php if($getstuid->gender_type == 1){echo 'S/O';}else{echo 'D/O';}?> <b><?php echo yii::$app->common->getParentName($id); ?></b>, Resident of <?php echo strtoupper($getstuid->location1 .' '. $getstuid->city->city_name .' '.  $getstuid->district->District_Name .' '. $getstuid->province->province_name .' '. $getstuid->country->country_name) ?> </li>
  <li>has been enrolled in this school in standard, <?php echo $levInfo->fkEnrollClass->title?> OF <b><?php echo date('d M, Y',strtotime($getstuid->registration_date)) ?></b> under
registration No. <b><?php echo $users->username ?></b>, and left</li>
<li> on <?php echo date('d M',strtotime($levInfo->created_date)) ?> at standard <?= $levInfo->fkClass->title ?>. <?php echo ($getstuid->gender_type == 1)? 'His':'Her' ?> date of birth according to the school admission is: 
