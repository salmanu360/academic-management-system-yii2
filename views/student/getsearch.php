...
<input type="hidden" class="classcntct" value="" />
  ...
  //echo $getdropVal;die;
            ...
            <?= GridView::widget([
               ...
               // 'filterModel' => $searchModel,
                'columns' => [
                    ...
                    'value'=>function($data){
                    $getSession = StudentInfo::find()->where(['stu_id'=>$data->stu_id])->one();
                      if($getSession->session_id){
                      return $getSession->session->title;
                    ...
                    'headerOptions' => ['class' => 'parntClassHeader'],
                    'value'=>function($data){
                    $studetnDetails = Yii::$app->common->getStudent($data->stu_id);
                     return Yii::$app->common->getCGSName($studetnDetails->class_id,$studetnDetails->group_id,$studetnDetails->section_id);
                    }
                    ...
                    [
                        ...
                        'buttons' => [
                            ...
                            {
                                return Html::a('<span class="glyphicon glyphicon-eye-open toltip" data-placement="bottom" width="20"  title="View Student"></span>', ['student/profile','id'=>$model->stu_id],['target'=>'_blank','data-pjax'=>"0",'class'=>'btn btn-success btn-xs']);
                            },
                            'update' => function ($url, $model, $key)
                            {
                                return Html::a('<span class="glyphicon glyphicon-pencil toltip" data-placement="bottom" width="20"  title="Edit Student"></span>',Url::to(['student/update','id'=>$key],['target'=>'_blank','data-pjax'=>"0",'class'=>'btn btn-primary btn-xs']));
                            },
                            ...
                            {
                                return Html::a('<i class="fa fa-envelope-o btn-info btn-xs data-toggle="modal" data-target="#myModal" title="Send SMS"></i>','javascript:void(0);',['data-toggle'=>'modal','data-target'=>'#myModal','data-stu_id'=>$key,'id'=>'stu']);
                            }, 
                        ],
                    ],
                ],
        ...
        if($getdropVal == 'class'){
            ?>
            <input type="submit" name="Generate Report" value="Generate Report" class="btn btn-primary classpdf" data-url="<?=Url::to(['student/get-search-pdf']) ?>" />
            ...
            <?= GridView::widget([
                ...
                'columns' => [
                  ...
                  [
                    ...
                    'value'=>function($data){
                        if($data->class_id){
                      return Yii::$app->common->getCGSName($data->class_id,$data->group_id,$data->section_id);
                    ...
                    [
                       ...
                       // 'contentOptions' =>['style' => 'width:150px'],
                        ...
                        'buttons' => [
                            'ViewProfile' => function ($url, $model, $key)
                            {
                                return Html::a('<i class="fa fa-fw fa-eye" title="Student Profile"></i>', ['student/profile','id'=>$key],['target'=>'_blank','data-pjax'=>"0"]);
                            },
                            'update' => function ($url, $model, $key)
                            {
                                return Html::a('<i class="fa fa-fw fa-edit" title="Edit Student"></i>',['student/update','id'=>$key],['target'=>'_blank','data-pjax'=>"0"]);

                            },
                            'delete' => function ($url, $model, $key)
                            {
                                 return Html::a(Yii::t('yii', '<span class="glyphicon glyphicon-trash toltip" data-placement="bottom" title="In Active Student"></span>'), 'update-status/'.$key, [
                                    'title' => Yii::t('yii', 'update-status'),
                                    'aria-label' => Yii::t('yii', 'update-status'),
                                    'onclick' => "
                                        if (confirm('Are You Sure You Want To In active this user...?')) {
                            ...
                            'leaving' => function ($url, $model, $key)
                            {
                                ...
                                ?>
                            <?php 
                            return '<i class="fa fa-external-link leavingBtn" data-placement="bottom"   title="Leaving Institution" data-toggle="modal" data-target="#myModalClass" data-stuid="'.$key.'"></i>' ?>
                            <?php
                             } },
                        ],
                    ],
    ...
    <input type="hidden" name="" id="getStudid" value="">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
            ...
            <?php
         }// end of contact and start of alumni
          if($getdropVal == 'alumni'){
            ?>
            <input type="submit" name="Generate Report" value="Generate Report" class="btn btn-primary classpdf" data-url="<?=Url::to(['student/get-alumni-pdf']) ?>" />
            <div class="row">
            <div class="col-md-12">
            <?= GridView::widget([
                'dataProvider' => $dataprovider,
                ...
                'columns' => [
                    ...
                    'headerOptions' => ['class' => 'fullnameClassHeader'],
                    'value'=>function($data){
                    $getStudentId=StudentInfo::find()->where(['stu_id'=>$data->stu_id])->one();
                     return Yii::$app->common->getName($getStudentId->user_id);
                    },
                     'visible' => true,
                    ],
                    ...
                    [
                        'header'=>'Actions',
                        'class' => 'yii\grid\ActionColumn',
                        'template' => "{SendSms}{ViewProfile}{active}",
                        'buttons' => [
                            'ViewProfile' => function ($url, $model, $key)
                            {
                                return Html::a('<i class="fa fa-fw fa-eye" title="Student Profile"></i>', ['student/profile','id'=>$model->stu_id],['target'=>'_blank','data-pjax'=>"0"]);
                            },
                            'active' => function ($url, $model, $key)
                            {
                                $link='student/activestu'.'?id='.$key;
                                 return Html::a(Yii::t('yii', '<span class="glyphicon glyphicon-ok" data-placement="bottom" title="Activate"></span>'), 'student/activestu/'.$key, [
                            ...
                            'SendSms' => function ($url, $model, $key)
                            {
                                return Html::a('<i class="fa fa-envelope-o data-toggle="modal" data-target="#myModal" title="Send SMS"></i>','javascript:void(0);',['data-toggle'=>'modal','data-target'=>'#myModal','data-stu_id'=>$model->stu_id,'id'=>'stu']);
                            },
                        ],
                    ],
                ],
         ...
         // end of alumni and start of name
        if($getdropVal == 'reg' ||  $getdropVal == 'name'){?>
    <input type="submit" name="Generate Report" value="Generate Report" class="btn btn-primary classNamepdf" data-url="<?=Url::to(['student/get-searchname-pdf']) ?>" />
    <div class="table-responsive">
            ...
            'columns' => [
                ...
                [
                    ...
                    'headerOptions' => ['class' => 'fullnameClassHeader'],
                    'value'=>function($data){
                    $getSession = StudentInfo::find()->where(['user_id'=>$data->id])->one();
                      if($getSession->session_id){
                      return $getSession->session->title;
                ...
                'value'=>function($data){
                    ...
                    'value'=>function($data,$key){
                     ...
                     if(!empty($cls)){
                      return Yii::$app->common->getCGSName($cls->class_id,$cls->group_id,$cls->section_id);
                     }else{
                        return "N/A";
                     }
                    }
                ...
                [
                    ...
                    'buttons' => [
                        'update' => function ($url, $model, $key)
                        {
                            ...
                            return Html::a('<span class="glyphicon glyphicon-pencil toltip" data-placement="bottom" width="20"  title="Edit Student"></span>',['student/update','id'=>$stu_id['stu_id']],['target'=>'_blank','data-pjax'=>"0",'class'=>'btn btn-primary btn-xs']);
                        },
                        'leaving' => function ($url, $model, $key)
                            {
                                ...
                                ?>
                            <?php return '<i class="fa fa-check-square btn btn-danger btn-xs leavingBtnName" data-placement="bottom" width="20"  title="Leaving Institution" data-toggle="modal" data-target="#myModalName" data-stuid="'.$stu_id['stu_id'].'"></i>' ?>
                            <?php  } },
                        'delete' => function ($url, $model, $key)
                        {
                        ...
                        'SendSms' => function ($url, $model, $key)
                            {
                                $stu_id=StudentInfo::find()->where(['fk_branch_id'=>Yii::$app->common->getBranch(),'user_id'=>$model->id])->one();
                                 if(count($stu_id)>0){
                                     return Html::a('<i class="fa fa-envelope-o btn btn-info btn-xs data-toggle="modal" data-target="#myModal" title="Send SMS"></i>','javascript:void(0);',['data-toggle'=>'modal','data-target'=>'#myModal','data-stu_id'=>$stu_id->stu_id,'id'=>'stu']);
                                 }else{
                                     return Html::a('<i class="fa fa-envelope-o data-toggle="modal" data-target="#myModal" title="Send SMS"></i>','javascript:void(0);',['data-toggle'=>'modal','data-target'=>'#myModal','data-stu_id'=>$key,'id'=>'stu']);
                                 }
                        ...
                        'ViewProfile' => function ($url, $model, $key)
                        {
                            ...
                            if(count($stu_id)>0){
                                return Html::a('<span class="glyphicon  glyphicon-eye-open" data-placement="bottom" width="20"  title="View Profile"></span>', ['student/profile','id'=>$stu_id->stu_id],['target'=>'_blank','data-pjax'=>"0",'class'=>'btn btn-success btn-xs']);
                            }else{
                ...
                'columns' => [
                    ...
                    [
                        ...
                        'buttons' => [
                            'ViewProfile' => function ($url, $model, $key)
                            {
                                return Html::a('<span class="glyphicon glyphicon-eye-open toltip" data-placement="bottom" width="20"  title="View Student"></span>', ['student/profile','id'=>$key],['target'=>'_blank','data-pjax'=>"0",'class'=>'btn btn-success btn-xs']);
                            },
...
Modal::begin([
    ...
    'footer'=>'<button type="button" class="btn btn-primary pull-left" id="close-generate-pdf" data-dismiss="modal">Close</button><button class="btn green-btn pull-right" id="print-pdf" data-url="'.Url::to(['student/generate-dmc-pdf']).'">Generate Pdf</button>',
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <input type="text" name="getstudent_id" id="stu_id" value=""/>
        </div>
        <div class="modal-body">
...
$(document).on('click','.saveLeavingBtn',function(e){
     e.preventDefault()
     e.stopImmediatePropagation(); 
