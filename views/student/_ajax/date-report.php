...
<h3 style='text-align:center'>Student Attendance report of <?php echo Yii::$app->common->getCGSName($class_id,$group_id,$section_id) ?> (<?= date('M-Y',strtotime($start)) ?>)</h3>
<?php }else{?>
  <div class="row">
  <div class="col-md-12">
    <a href="<?=Url::to(['student/cal-show','date'=>$start,'c_id'=>$class_id,'g_id'=>$group_id,'s_id'=>$section_id]) ?>" name="Generate Report" class="btn btn-primary pull-right"><i class="fa fa-download"></i> Generate Report</a>
  </div>
</div>
<?php } ?>
  ...
  <div class="col-md-12">
    ...
    <table class="table table-bordered">
      ...
      <tbody>
       ...
       foreach ($stuQuery as $key => $stuQuery) {
         ?> <tr>
          <td><?php echo $stuQuery['name'] ?></td>
          <td><?php echo $stuQuery['roll_no'] ?></td>
          <?php foreach ($list as $key => $getDate) {?>

           <td><?php 
           $attendance=\app\models\StudentAttendance::find()->where(['fk_stu_id'=>$stuQuery['stu_id'],'date'=>$getDate])->one();?>
