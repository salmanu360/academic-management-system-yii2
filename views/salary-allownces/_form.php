...
$groups = ArrayHelper::map(\app\models\SalaryPayGroups::find()->where(['fk_branch_id'=>yii::$app->common->getBranch()])->all(), 'id', 'title');
                    echo $form->field($model, 'fk_pay_group')->widget(Select2::classname(), [
                        'data' => $groups,
                        'options' => ['placeholder' => 'Select Pay Head ...','data-url'=>Url::to(['salary-allownces/get-stages']),'class'=>'groups'],
                        'pluginOptions' => [
                            'allowClear' => true
                        ],
                    ])->label('Pay Head');
       ...
       if($model->isNewRecord == 1){
        ...
        $stage = ArrayHelper::map(\app\models\SalaryPayStages::find()->where(['fk_branch_id'=>yii::$app->common->getBranch()])->all(), 'id', 'title');
                    echo $form->field($model, 'fk_stages_id')->widget(Select2::classname(), [
                        //'data' => $groups,
                        'options' => ['placeholder' => 'Select Pay Head Type ...','class'=>'getstage'],
                        'pluginOptions' => [
                            'allowClear' => true
                        ],
                    ])->label('Pay Head Type');
