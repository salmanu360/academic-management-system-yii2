    ...
    <div id="showPreviousDetails">
     ...
     if($head_arrears_previous== null){$head_arrears_previous=0;}
      ...
      if(!empty($PreviousFeeTakenMonth['from_date'])){
          $total_transport_arrears = $PreviousFeeTakenMonth['transport_arrears'];
          $total_hostel_arrears=$PreviousFeeTakenMonth['hostel_arrears'];
     ...
     <div class="table-responsive">
        <table class="table table-hover">
          ...
          <tbody>
            ...
            $getDateFeesCount=\app\models\FeeSubmission::find()->select(['year_month_interval'])->where(['stu_id'=>intval($student_id),'branch_id'=>yii::$app->common->getBranch(),'fee_head_id'=>$getFeeDetails->fk_fee_head_id])->asArray()->all();
        ...
        $interval_concat .= $month_year_int['year_month_interval'];
            ...
            if(!in_array($fromConvert, $explode_interval) && !in_array($toConvert, $explode_interval)){
             ...
             /*new query by salman for trasport fee show when take admission submit*/
              /*$getAdmissionHead=\app\models\FeeHead::find()->where(['branch_id'=>yii::$app->common->getBranch(),'one_time_payment'=>1,'promotion_head'=>0])->one();
              $getAdmissionFee=\app\models\FeeSubmission::find()->select(['head_recv_amount','transport_amount'])->where(['fee_head_id'=>$getAdmissionHead->id,'stu_id'=>intval($student_id),'transport_amount'=>0,'fee_status'=>1])->one();
                if(count($getAdmissionFee) == 0){
                $transport_amount=0;
                 $hostel_amount=0;
                }*/
                 
               
             /*new query by salman ends*/
               $getDateFees=0;
               /*$transport_amount=0;
               $hostel_amount=0;*/
            ...
            if(count($getDateFees) > 0){
              ...
              /*if promotion head is active*/ 
               $year=date('Y');  
              $promotedData=\app\models\StuRegLogAssociation::find()->where(['fk_stu_id'=>$student_id])->andWhere(['YEAR(promoted_date)'=>$year])->count();
