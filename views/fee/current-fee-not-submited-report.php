	...
	?>
		<tr>
			...
			/*if promotion head is active*/ 
               ...
               $oldStudent=\app\models\StuRegLogAssociation::find()->where(['fk_stu_id'=>$values['stu_id']])->count();
               $year=date('Y');
               $promotedData=\app\models\StuRegLogAssociation::find()->where(['fk_stu_id'=>intval($values['stu_id'])])->andWhere(['>=','date(promoted_date)',$sessionStartDate])->andWhere(['<=','date(promoted_date)',$sessionEndDate])->count();
              if($getHead->promotion_head == 1){
                $promotionFeeReceive =\app\models\FeeSubmission::find()->where(['stu_id'=>$values['stu_id'],'branch_id'=>yii::$app->common->getBranch(),'fee_head_id'=>$getFeeDetailsvalue['fk_fee_head_id'],'YEAR(recv_date)'=>$sessionEndDate])->sum('head_recv_amount');
              ...
              /*if promotion head is active ends*/
               /*temproary avoid admission fee*/
                ...
                if($getHead->one_time_payment == 1 && $getHead->promotion_head ==0 && $promotedData > 0){
                  continue;
                }

                if($getHead->one_time_payment == 1 && $getHead->promotion_head ==0 && $promotedData == 0 && $oldStudent > 0){
                  continue;
                }
             ...
             if($values['avail_sibling_discount']==1 && $getFeeDetailsvalue['fk_fee_head_id'] == $getHead['id'] && $getHead['sibling_discount'] ==1 ){
                     ...
                     $amount=$getFeeDetailsvalue['amount']-$sibling_amount;
					}else{
