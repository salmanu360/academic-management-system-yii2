...
if($absent_count >0){ 
 ...
 <div class="box box-warning">
    ...
    <input type="button" class="btn btn-warning view-button" data-url="<?php echo Url::to(['fee/previous-details-student','id'=>$student_id]) ?>" value="Session fee details">
    ...
    // echo $promotedData;die;
              ...
              if($getHead->promotion_head == 1){
               ...
               ->andWhere(['between', 'YEAR(recv_date)', $sessionStartDate, $sessionEndDate])
                ...
                /*$promotionFeeReceive =\app\models\FeeSubmission::find()->where(['stu_id'=>intval($student_id),'branch_id'=>yii::$app->common->getBranch(),'fee_head_id'=>$getFeeDetails->fk_fee_head_id,'YEAR(recv_date)'=>$sessionEndDate])
                ->sum('head_recv_amount');*/
...
$(".view-button").click(function() {
        $.get($(this).attr("data-url"), function(data, status){
            $('#myModalFee').html(data);
            $('#myModalFee').modal('show');
        });
    });
</script>
<div class="modal fade" id="myModalFee" tabindex="-1" role="dialog" aria-hidden="true">
</div>