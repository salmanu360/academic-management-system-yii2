...
$firstClass=StuRegLogAssociation::find()->where(['fk_stu_id'=>$id])->one();
...
<body>
  ...
  <h2 style="font-size:16px; font-weight:700; color:#037dbd;margin: 0;padding: 23px 0 8px 0;">
  ...
  font-weight: 700;
  color:#037dbd;
  padding-top: 20px;"> CHARACTER CERTIFICATE</div> 
  <!-- student image -->
  <?php if(!empty($file_name) && file_exists($file_path.$file_name)) {
      ...
      <img style="margin-left:30px;margin-top: -70px" width="85px;" height="71px" src="<?= $web_paths.$imageNameStudent?>" alt="<?=Yii::$app->common->getName($getstuid->user->id);?>">
      <?php }else{?>
  <img style="margin-left:30px;margin-top: -70px" width="85px" height="71px" src="<?= Url::to('@web/img')?>/<?=($getstuid->gender_type == 1)? 'male.jpg' :'female.png' ?>" alt="<?=Yii::$app->common->getName($getstuid->user->id);?>">
             <?php }?>
...
<div class="text" style="margin-left: 20px;
  ...
  <ol><i>
    ...
    <li> has been enrolled in standard ,<?php echo (empty($firstClass->fk_stu_id))? $getstuid->class->title : $firstClass->class->title; ?> OF <b><?php echo date('d M, Y',strtotime($getstuid->registration_date)) ?></b> under
registration No. <b><?php echo $users->username ?></b> , and left on</li>
<li> <?php echo date('d M',strtotime($levInfo->created_date)) ?> at standard <?= $levInfo->class->title ?>. <?php echo ($getstuid->gender_type == 1)? 'His':'Her' ?> date of birth according to the school admission is: 
In Figure : <b><?php echo date('d M Y',strtotime($getstuid->dob)) ?></b>.</li>
<li><?php echo ($getstuid->gender_type == 1)? 'He':'She' ?> has been found abiding by the all rules and regulations of the Institute
and performed well in <?php echo ($getstuid->gender_type == 1)? 'his':'her' ?> academic and co-curricular </li>
<li> activities.</li>
<li>
<?php echo ($getstuid->gender_type == 1)? 'His':'her' ?> Conduct and character as recorded by the Institute discipline committee during <?php echo ($getstuid->gender_type == 1)? 'his':'her' ?> stay in the Institute has been satisfictory.</li>
</i></ol></div>
