...
<h3 style='text-align:center'>Fee Ledger Of <?= yii::$app->common->getName($studentTable->user_id)  ?> <?= ($studentTable->gender_type== '0')? 'D/O' : 'S/O' ?> <?= Yii::$app->common->getParentName($studentTable->stu_id).' (Class '.$studentInfo->class->title .') in '.date('M Y',strtotime($month));?>
</h3>
<?php }else{?>
<a href="<?=\yii\helpers\Url::to(['reports/monthly-report-classwise-pdf/','month'=>$month,'student_id'=>$student_id]) ?>" name="Generate Report" class="btn btn-primary pull-right"><i class="fa fa-download"></i> Generate Report</a>
                        ...
                        <thead>
                           ...
                           <tbody>
                            ...
                            foreach ($yearSql as $key => $currentYear) {
                                $i++; 
                                $where="from_date like '".$month."%' and fee_head_id=".$currentYear->fee_head_id." and stu_id=".intval($currentYear->stu_id);
                             ...
                             $whereArrears="from_date like '".$month."%' and fee_head_id=".$currentYear->fee_head_id." and status=1 and stu_id=".intval($currentYear->stu_id);
