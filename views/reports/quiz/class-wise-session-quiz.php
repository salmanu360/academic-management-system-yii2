...
<div class="row">
	<div class='col-md-12'>
		<h5 style="color:red">Quiz report of Class (<?php echo $refClass->title ?>),  Session: <?php echo date('Y',strtotime($sessionStart)) ?> - <?php echo date('Y',strtotime($sessionEnd)) ?>
	<?php if(!isset($_GET['class_id'])){?>
	<a style="margin-top: -10px" href="<?= Url::to(['reports/class-wise-quiz-session','class_id'=>$class_id]) ?>" class="btn btn-primary btn-sm pull-right">Generate Report</a>
	<?php } ?>
</h5>
	</div>
</div>
...
<tbody>
	...
	<tr>
		...
		<td><span style="color:#293ca9;text-decoration: underline; cursor: pointer;	" id="classWiseDateQuiz" data-url='<?php echo Url::to(['subject-wise-date-quiz']) ?>' data-testid='<?php echo $exam_quiz_type_detailsvalue->id ?>'><?php echo $exam_quiz_type_detailsvalue->quiz_date ?></span></td>
	</tr>
	<?php } ?>
</tbody>
</table>
