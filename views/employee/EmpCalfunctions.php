...
?>
<div id="calender_section" class="fullCalendar table-details shade attendance">
  <div class="row">
    <div class="col-md-12">
...
<div class="row">
  ...
  <table class="table table-striped table-bordered" cellspacing="0" width="100%">  
        ...
        <?php 
            ...
            for($cb=1;$cb<=$boxDisplay;$cb++){
                if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
                    ...
                    echo '<span>';
                    //echo $dayCount;
                    echo $offday= date('D',strtotime($currentDate));
                    $daysSet=WorkingDays::find()->select('title')->where(['is_active'=>0])->All();
        ...
        $emp=EmployeeInfo::find()->where(['fk_branch_id'=>Yii::$app->common->getBranch(),'is_active'=>1])->All();
        foreach ($emp as $q) {
         //echo count($emp);
        ...
        //echo '<pre>';print_r($GetWorkingdays);
                ?>
                ...
                // echo $mydate=date('Y-m-d',strtotime($currentDate));
                    ...
                    <?php }?>
                 <?php 
                 //if($ss <= $dd && $mm >= $mcalendr && $offdayscheck == $offSettingDay) 
                 if($offdayscheck == $offSettingDay) // previos day remove

                  { ?>
                
                <a href="" data-toggle="modal" data-target="#myModal" data-empid="<?php echo $q->emp_id;?>" data-date="<?php echo $currentDate .' '. date('H:i:s')?>" class="popups" id="emp_<?php echo $q->emp_id.date('d',strtotime($currentDate))?>" data-urls=<?php echo yii\helpers\Url::to(['employee/save-emp-id'])?>>
                &nbsp;&nbsp;&nbsp;&nbsp;
                <?php }else{
                   //echo 'off';
                  // echo '<span style="background: black;">asdf</span>';
                }
                }
                        ...
                        //echo $currentDate;
                             ...
                             if($getCurrentD == $date){
                                     ...
                                     if($attend->leave_type == 'absent'){
                                        echo '<span id="emp_'.$q->emp_id.date('d').'" class="at_absent at_sp">A</span>';
                                     }else if($attend->leave_type == 'leave'){
                                        echo '<span id="apends" id="emp_'.$q->emp_id.date('d').'" class="leaveid at_sp at_leave">L</span>';
                                     }else if($attend->leave_type == 'latecomer'){
                             ...
                             // echo '<span style="color:#800000">'.date('H:i',strtotime($attend->date)).'</span>';
                                  ...
                                  echo '<span  class="at_sleave at_sp">SL</span>';
                                     }
                                     else if($attend->leave_type == 'present'){
                                     echo '<span class="policies at_sp at_leave_present">P</span>';
                            //echo '<span style="color:#800000">'.date('H:i',strtotime($attend->date)).'</span>';
                                     }
                                     else if($attend->leave_type == 'present' and strtotime($latetime) > '7:00:00'){
                                        echo '<span style="color:#800000">'.date('H:i',strtotime($attend->date)).'</span>';
                                     }
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <?php
          ...
          $a= ['absent' => 'Absent', 'leave' => 'Leave','shortleave'=>'Short Leave', 'latecomer' => 'Late Commer','present'=>'Present'];
		...
		<?php echo Html::label('Remarks'); ?>
          ...
          <label for="" class="displayvalidationRemarks" style="color: red"></label>
